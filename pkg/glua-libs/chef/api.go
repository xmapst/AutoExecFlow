...
func NewClient(L *lua.LState) int {
	...
	if L.GetTop() > 3 {
		...
		if v, ok := ud.Value.(luahttp.LuaHTTPClient); ok {
			chefClient.LuaHTTPClient = v
		} else {
			L.ArgError(2, "must be http_client_ud")
		}
	} else {
		chefClient.LuaHTTPClient = luahttp.NewPureClient()
	}
	ud := L.NewUserData()
	ud.Value = chefClient
...
func Request(L *lua.LState) int {
	...
	value, err := luajson.ValueDecode(L, responseBytes)
	if err != nil {
		L.Push(lua.LNil)
		L.Push(lua.LString(err.Error()))
		return 2
...
func Search(L *lua.LState) int {
	...
	if partical != nil {
		data, err := luajson.ValueEncode(partical)
		if err != nil {
			L.Push(lua.LNil)
			L.Push(lua.LString(err.Error()))
			return 2
