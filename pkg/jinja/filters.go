...
func filterFile(e *exec.Evaluator, in *exec.Value, params *exec.VarArgs) *exec.Value {
	...
	if !filepath.IsAbs(_path) {
		...
		_path, err = filepath.Abs(filepath.Join(base, _path))
		if err != nil {
			return exec.AsValue(fmt.Errorf("failed to resolve path %s with loader: %s", _path, err))
		}
	}

	out, err := os.ReadFile(_path)
	if err != nil {
		return exec.AsValue(fmt.Errorf("failed to read file at path %s: %s", _path, err))
	}
