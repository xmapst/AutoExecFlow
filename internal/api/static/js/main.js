        ...
        const menu = new G6.Menu({
            ...
            getContent(e) {
                ...
                if (step.state === 'running') {
                    menuContent += '<a href="#" id="killTask">强杀</a>';
                } else if (step.state === 'paused') {
                    menuContent += '<a href="#" id="resumeTask">解挂</a>';
                    menuContent += '<a href="#" id="killTask">强杀</a>';
                } else if (step.state === 'pending') {
                    menuContent += '<a href="#" id="pauseTask">挂起</a>';
                    menuContent += '<a href="#" id="killTask">强杀</a>';
                }
                if (menuContent === '') {
                    return '无操作可选';
...
class TaskAddCard {
        ...
        card.innerHTML = `
            <div class="card-header">
                <div class="button" style="position: fixed; top: 6px; right: 12px;">
                    <button id="createTask" class="button-sure">创建</button>
                    <button id="cancelModal" class="button-cancel">取消</button>
                </div>
            </div>
    ...
    initializeEditor() {
        ...
        require(['vs/editor/editor.main'], () => {
            this.editor = monaco.editor.create(document.getElementById('yamlEditor'), {
                value: taskTpl,
                language: 'yaml',
                theme: 'vs-dark',
                autoIndent: true,
    ...
    renderTable() {
        ...
        if (!this.tasks || this.tasks.length === 0) {
            ...
            this.tasks.forEach(task => {
                    ...
                    <td id="${task.name+'-actions'}">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-content">
                                ...
                                ${task.state === 'running' ? '<a href="#" id="killTask">强杀</a>' : ''}
                                ${task.state === 'stopped' || task.state === 'failed' ? '<a href="#" id="deleteTask">删除</a>' : ''}
                            </div>
                        </div>
                    </td>
                `;
                ...
                if (row.querySelector("#killTask") !== null) {
                    row.querySelector("#killTask").addEventListener("click", () => Utils.taskManager(task.name, 'kill'));
                }
                if (row.querySelector("#deleteTask") !== null) {
    ...
    createMainContent() {
        ...
        mainDiv.innerHTML = `
            <div id="container" class="container">
                <div class="header">
                    <div class="button">
                        <button id="addTask" class="button-sure">添加</button>
                    </div>
                </div>
                ...
                <div class="pagination">
                    <div style="margin-right: 6px;">
                        <button id="prevPage" class="button-sure">上一页</button>
                        <span id="pageInfo">第1页__共1页</span>
                        <button id="nextPage" class="button-sure">下一页</button>
                    </div>
